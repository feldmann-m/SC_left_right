...
for file in c_c_files:
    ...
    row = {
        ...
        'delta_Z500a': b2.FI.sel(pressure=500).values[0]-b3.FI.sel(pressure=500).values[0],
        'delta_Z500b': b1.FI.sel(pressure=500).values[0]-(data.FI.sel(x=0,y=0)).values[0],
        ...
        'area_prec': (data.TOT_PREC>(0.1/12)).mean(dim=('x','y')).values[0],
        'max_gust': b2.VMAX_10M.values[0],
        'mean_gust': b1.VMAX_10M.values[0],
        'area_gust': (data.VMAX_10M>12).mean(dim=('x','y')).values[0],
        'area_gust1': (data.VMAX_10M>17.2).mean(dim=('x','y')).values[0],
        'area_gust2': (data.VMAX_10M>20.8).mean(dim=('x','y')).values[0],
        'max_hail': b2.DHAIL_MX.values[0],
        'mean_hail': b1.DHAIL_MX.values[0],
        'area_hail': (data.LPI_MAX>0.1).mean(dim=('x','y')).values[0],
        'area_hail1': (data.LPI_MAX>1).mean(dim=('x','y')).values[0],
        'area_hail5': (data.LPI_MAX>5).mean(dim=('x','y')).values[0],
        'area_hail10': (data.LPI_MAX>10).mean(dim=('x','y')).values[0],
...
df_r.to_csv('/storage/homefs/mf23m219/SC_env/newdisk_'+reg+'_'+cl+'_t-'+str(t)+'_RM_td.csv')
df_l.to_csv('/storage/homefs/mf23m219/SC_env/newdisk_'+reg+'_'+cl+'_t-'+str(t)+'_LM_td.csv')
