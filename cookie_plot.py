...
plotter = importlib.reload(plotter)

def set_colorbar_label_old(clb, stat, label, unit, delta):
    ...
    if delta == True:
        clb.set_label(f"$\\Delta$ {label} [{unit}]\n({stats_mapping.get(stat, stat)})", fontsize = 1.5 * pu.font_size())
    else:
        clb.set_label(f"{label} [{unit}]\n({stats_mapping.get(stat, stat)})", fontsize = 1.5 * pu.font_size())
        
def set_colorbar_label(clb, stat, s_label, unit, delta):
    ...
    if delta == True:
        clb.set_label(f"$\\Delta$ {s_label} [{unit}]", fontsize = 12)#1.25 * pu.font_size())
    else:
        clb.set_label(f"{s_label} [{unit}]", fontsize = 12)#1.25 * pu.font_size())
...
def main(relative_time, subdomain, season, stat, variable, label, unit, cmap, direction, pressure_level = None, boundaries=None, boundaries_delta=None, cmap_delta=None, ptest=False):
    ...
    else: p = np.zeros(vals_surf_delta.shape)
...
def main_2(relative_time, subdomain, season, stat, variable, label, s_label, unit, cmap, direction, pressure_level = None, boundaries=None, boundaries_delta=None, cmap_delta=None, ptest=False, rad=60):
    import cookie_plotter as plotter
    plotter = importlib.reload(plotter)
    signature = 1 if direction == 'right' else -1
    nice_rounding = False
    ...
    fig, axes = plt.subplots(1, 2, figsize = (pu.two_column(), pu.two_column()))
    ...
    if variable == 'FI':
        ...
        vals_surf_present = vals_surf_present * 12
        vals_surf_future = vals_surf_future * 12
    ...
    # plotter.text_on_circle_counterclock(axes[0], rad*1.15, 58.5, subdomain + ',' + f'ENV-{relative_time}h')
    # plotter.text_on_circle(axes[0], rad*1.15, 24, str(cookies_present) + f' {direction} movers')
    # axes[0].set_title('CTRL', pad = 15, fontsize = 2 * pu.font_size())
    ...
    # plotter.text_on_circle_counterclock(axes[1], rad*1.15, 58.5, subdomain + ',' + f'ENV-{relative_time}h')
    # axes[1].set_title('$\Delta=$' + 'PGW' + '$-$' + 'CTRL', pad = 15, fontsize = 2 * pu.font_size())
    
    ax0 = axes[0].get_position().get_points().flatten()
    ax1 = axes[1].get_position().get_points().flatten()
    #ax2 = axes[2].get_position().get_points().flatten()
    ax_cbar = fig.add_axes([ax0[0]-0.04, ax0[1], 0.02, ax0[3] - ax0[1]])
    ax_cbar_delta = fig.add_axes([ax1[2]+0.01, ax1[1], 0.02, ax1[3] - ax1[1]])
    clb = plt.colorbar(cm.ScalarMappable(norm = norm, cmap = cmap), cax = ax_cbar, orientation = 'vertical')
    clb.set_ticks(boundaries[::2])
    clb.minorticks_off()
    clb.ax.yaxis.set_ticks_position('left')
    clb.ax.yaxis.set_label_position('left')
    clb.ax.tick_params(rotation = 0)
    set_colorbar_label(clb, stat, s_label, unit, False)
    clb_delta = plt.colorbar(cm.ScalarMappable(norm = norm_delta, cmap = cmap_delta), cax = ax_cbar_delta, orientation = 'vertical')
    clb_delta.set_ticks(boundaries_delta[::3])
    clb_delta.minorticks_off()
    clb_delta.ax.tick_params(rotation = 0)
    clb.ax.tick_params(labelsize=12) 
    clb_delta.ax.tick_params(labelsize=12) 
    set_colorbar_label(clb_delta, stat, s_label, unit, True)
...
def main_1(relative_time, subdomain, season, stat, variable, label, s_label, unit, cmap, direction, pressure_level = None, boundaries=None, boundaries_delta=None, cmap_delta=None, ptest=False, rad=60, cont_stat=None, cont_variable=None, cont_bounds=None, cont_bounds_d=None, plevel_c=None):
    import cookie_plotter as plotter
    plotter = importlib.reload(plotter)
    signature = 1 if direction == 'right' else -1
    nice_rounding = False
    dr='R' if direction == 'right' else 'L'
    ...
    if cont_variable=='FI': relative_time_c=2
    else: relative_time_c=relative_time
    present_cont = glob(f'/storage/workspaces/giub_meteo_impacts/ci01/supercell_climate/cookies/present_t-{relative_time_c}/comp_{direction}_movers/{subdomain}_{season}_signature{signature}_{cont_stat}*')[0]

    future_cont = glob(f'/storage/workspaces/giub_meteo_impacts/ci01/supercell_climate/cookies/future_t-{relative_time_c}/comp_{direction}_movers/{subdomain}_{season}_signature{signature}_{cont_stat}*')[0]

    
    # extract the number of cookies used for the composite
    pattern = r'_n(\d+)\.nc$'
    match_present_cont = re.search(pattern, present_cont)
    match_future_cont = re.search(pattern, future_cont)
    cookies_present_cont = match_present_cont.group(1)
    cookies_future_cont = match_future_cont.group(1)

    with xr.open_dataset(present_cont) as present_c:
        pass
    
    with xr.open_dataset(future_cont) as future_c:
        pass

    present_c = present_c.drop_vars('cookie_id')
    present_c = present_c.squeeze()
    
    future_c = future_c.drop_vars('cookie_id')
    future_c = future_c.squeeze()

    vals_surf_present_c = present_c[cont_variable] if plevel_c == None else present_c[cont_variable].sel(pressure = plevel_c)
    vals_surf_future_c = future_c[cont_variable] if plevel_c == None else future_c[cont_variable].sel(pressure = plevel_c)
    if cont_variable == 'FI':
        vals_surf_present_c = mpcalc.geopotential_to_height(vals_surf_present_c) / 10
        vals_surf_future_c = mpcalc.geopotential_to_height(vals_surf_future_c) / 10
    elif cont_variable == 'QV_2M' or variable == 'QV':
        vals_surf_present_c = vals_surf_present_c * 1000
        vals_surf_future_c = vals_surf_future_c * 1000
    elif cont_variable == 'TOT_PREC':
        vals_surf_present_c = vals_surf_present_c * 12
        vals_surf_future_c = vals_surf_future_c * 12
    vals_surf_delta_c = vals_surf_future_c - vals_surf_present_c

    fig, axes = plt.subplots(1, 1, figsize = (pu.one_column(), pu.one_column()))
    ...
    t_contour = plotter.plt_composite(
        ...
        axes,
        vals_surf = vals_surf_present,
        vals_cont = vals_surf_present_c,
        contour_levels = cont_bounds,
        #vals_quiver = (present['U_10M'], present['V_10M']),
        #quiver_color = 'black',
        #quiver_scale = 20,
        #quiver_key_scale = 5,
    ...
    # plotter.text_on_circle_counterclock(axes, rad*1.15, 58.5, subdomain + ',' + f'ENV-{relative_time}h')
    # plotter.text_on_circle(axes, rad*1.01, 24, str(cookies_present) + f' {dr}M')
    #axes.set_title('CTRL', pad = 15, fontsize = 2 * pu.font_size())
    ...
    #     axes[1],
    #     vals_surf = vals_surf_delta,
    #     vals_cont = vals_surf_delta_c,
    #     contour_levels = cont_bounds_d,
    #     #vals_quiver = (future['U_10M'] - present['U_10M'], future['V_10M'] - present['V_10M']),
    #     #quiver_color = 'black',
    #     #quiver_scale = 5,
    #     #quiver_key_scale = 1,
    ...
    #ax2 = axes[2].get_position().get_points().flatten()
    ax_cbar = fig.add_axes([0.05, 0.1, 0.02, 0.8])#[0, 0.08, 1, 0.015])
    clb = plt.colorbar(cm.ScalarMappable(norm = norm, cmap = cmap), cax = ax_cbar, orientation = 'vertical')#'horizontal')
    clb.ax.yaxis.set_ticks_position('left')
    clb.ax.yaxis.set_label_position('left')
    clb.set_ticks(boundaries[::2])
    clb.ax.tick_params(labelsize=12) 
